...
void setup(){
  ...
  //LED pins
  pinMode(7,OUTPUT); //Guitar is sharp (Positive motor voltage)
  pinMode(6,OUTPUT); //Guitar is flat (Negative motor voltage)
  pinMode(5, OUTPUT); //ENABLE PIN
  
 digitalWrite(7,0);//motor off
 digitalWrite(6,0);//motor off
 digitalWrite(5,1);//enable on
...
//Sets the correct frequency for us to compare to by checking
//if sample falls between a range a frequencies
void stringDetermine(){
...
//Compare the frequency input to the correct frequency 
//determined above and output a voltage to control motor.
void motorTurn(){
   if((frequency > 500) || (frequency < 60)){
    digitalWrite(7, 0); 
    digitalWrite(6, 0);
    digitalWrite(5, 0);
   }
